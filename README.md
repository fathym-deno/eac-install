...
- **Goal:** provide commands for installing/configuring EaC assets across
  projects and runtimes.
- **Outputs:** CLI entrypoints (`install.ts`, `src/commands/**`) and supporting
  config/templates under `install/` and `config/`.
- **Code location:** this folder hosts the source.

## Current Status

- Commands located in `src/commands/**`; installer configs/templates under
  `install/` and `config/`.
- Tasks: `deno task test`, `deno task test:installer`, `deno task build`,
  `deno task publish:check`, `deno task deploy`, `deno task version`.
...
3. Capture provenance and release channels in `UPSTREAM.md` when publishing to
   jsr/npm; note install target assumptions.
